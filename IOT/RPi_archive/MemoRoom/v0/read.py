...
discoverable = False
started=None
subprocess.Popen(["sudo","bluetoothctl","power","off"])
...
while continue_reading:
    ...
    if status == MIFAREReader.MI_OK:
        ...
        if status == MIFAREReader.MI_OK:
            if discoverable==False:
                print ("###################### Card detected ######################")
                subprocess.Popen(["sudo","bluetoothctl","power","on"])
                sleep(1)
                process = subprocess.Popen(["sudo","bluetoothctl","discoverable","on"])
                
                process.wait(1)
                subprocess.Popen(["sudo","bluetoothctl","scan","on"])
                try:
                   process = subprocess.Popen(["sudo","bluetoothctl","pair","0C:B8:15:F8:6E:02"])
                   subprocess.Popen(["sudo","bluetoothctl","connect","0C:B8:15:F8:6E:02"])
                except:
                    pass
                discoverable=True
