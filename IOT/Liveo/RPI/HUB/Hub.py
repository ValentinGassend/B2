...
chromium_command = "chromium-browser --kiosk http://" + \
    str(address)+":"+str(webport)
...
class TestState(HubState):
    ...
    def communicate_via_websocket(self, hub, expected_message=None):
        ...
        if not self.previous_message == received_message:
            self.previous_message = received_message
...
class RecoveryState(HubState):
    ...
    def get_appointment_by_id(self, id):
        appointment = manager.get_appointment_by_id(id)
        return appointment
            ...
            if received_message:
                ...
                # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
                    ...
                    if not nlu_result:
                        ...
                        appointment = hub.get_appointment_by_id(
                            appointment_reminder['id'])
                        heure_rendez_vous = datetime.strptime(
                            appointment["heure"], "%H:%M")
                        heure_rappel = datetime.strptime(
                            appointment["rappel"]["heure"], "%H:%M")

                        difference_temps = heure_rendez_vous - heure_rappel
                        difference_heures = difference_temps.seconds // 3600
                        difference_minutes = (
                            difference_temps.seconds // 60) % 60

                        if difference_heures != 0:
                            temps_diff = f"{difference_heures} heures"
                        else:
                            temps_diff = ""

                        if difference_minutes != 0:
                            temps_diff += f" {difference_minutes} minutes"

                        print(f"Différence de temps : {temps_diff}")
                        hub.send_message_via_websocket(
                            "Whisper_remind data_OK")
                        hub.speak_text(
                            "Entendu. Vous souhaitez donc que je vous rappelle votre rendez-vous "+temps_diff+" avant ?")
                        hub.send_message_via_websocket("Whisper_bool start")
                        received_message = None
                        stepThree = False
                        stepFour = True
