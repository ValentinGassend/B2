...
class AppointmentManager:
    ...
    def transform_data_to_message(self, data):
        ...
        for i, appointment in enumerate(sorted_data, start=1):
            ...
            message += f"Le {num} rendez-vous est {title} à {appointment_time:%H:%M}.\n"
    ...
    def check_appointment(self, current_time):
        ...
        current_hours = current_time.time()
        current_hours = current_hours.strftime("%H:%M:%S")
        current_hours = current_hours[:6] + "00"
        # print(current_hours)
        ...
        for appointment in appointments:
            ...
            if appointment['rappel']['heure']:
                reminder_time = datetime.strptime(
                    appointment['rappel']['heure'], "%H:%M").time()
            # print("current_time.date()", current_time.date())
            # print("appointment_date", appointment_date)
            # print("appointment_date == current_time.date()",
            #       appointment_date == current_time.date())
            # print("current_hours", current_hours)
            # print("appointment_time", appointment_time)
            # print("current_hours == appointment_time()",
            #       appointment_time == current_hours)
            # print("current_hours", current_hours)
            # print("reminder_time", reminder_time)
            # print("reminder_time == current_hours",
            #       reminder_time == current_hours)
            # print("reminder_time == current_hours",
            #       str(reminder_time) == str(current_hours))
            if appointment_date == current_time.date() and str(appointment_time) == str(current_hours):
                self.set_reminder_apointment(appointment)
                return appointment
            elif appointment_date == current_time.date() and str(reminder_time) == str(current_hours):
                self.set_reminder_apointment(appointment)
